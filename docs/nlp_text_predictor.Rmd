...
This app's emphasis is quickly predicting a word and demonstrating the algorithm rather than to be as accurate as possible for all users.

## Data

The data at the time of publication of this app is roughly 150 MB of ngrams ranging from 1 to 4 words. All ngrams occurring only once are ignored to save memory. In the datasets, information looks like the following for bigrams and quadrigrams:
...
This app uses a modified Katz back-off algorithm to enhance speed and enable multiple predictions. This app's algorithm does not consider the "discount" probability introduced in the standard model, and so it does not consider unobserved ngrams when assigning probabilities to predictions.
...
The app has a simple interface where the user inputs text and can see the top prediction along with the rest of the top 20 to see what the algorithm can produce.
...
To get an idea of the app's performance, it has been put through a test set of a few thousand inputs with respective correct next words. The results show the app's accuracy and speed.

```{r echo=FALSE}
run_tests(3)
```

When outputting 3 predictions, only 20% of prediction sets had the correct answer, and the average runtime of the prediction was 0.04 seconds. There is a tradeoff here between accuracy and time: an algorithm with fewer ngrams drawn from the data and less computation gives less accurate predictions but does so more quickly. Using more data or using the computations from the Katz back-off model could improve the accuracy but will slow the application down.
